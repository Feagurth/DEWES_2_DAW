...
<html>
    <head>
        ...
        <title>Salida de documentos</title>
    </head>
    <body>
        <?php
         ...
         * @param type $fsalida Fecha de salida
         ...
         */
        function validarDatos($tipodoc, $fsalida, $remit, $dest) {
            ...
            // Verificamos que la fecha de salida no sea superior a la fecha 
            // actual puesto que no deberían permitirse la salida de documentos 
            // con dias posteriores al actual. No se puede registrar documentos 
            // que aún no han salido de la oficina
            if ($fsalida > date('Y-m-d')) {
                // Si la validación no se cumple, asignamos el valor 
                // correspondiente a la variable de salida                
                $validacion = 4;
            }
        ...
        function calcularNreg($bd) {
            ...
            // que haya en la tabla de salidas de la base de datos
            $nreg = $bd->query('Select max(nreg) from salidas');
        ...
        if (!isset($mensajeError)) {
            ...
            if (empty($_POST['dest'])) {
                ...
                $validacion = validarDatos($tipodoc, $fsalida, $remit, $dest);
                ...
                if ($validacion == 0) {
                    ...
                    $resultado = $gestion->exec("insert into salidas values("
                            . "0, "
                            . "$nreg, "
                            . "'$tipodoc' ,"
                            . "'$fsalida' ,"
                    ...
                    // Desasignamos el valor de fecha de salida
                    unset($fsalida);
                } else {
                ...
                $salida = $gestion->query('select * from salidas order by fsalida desc');
        ...
        <div id="nuevo_registro">
            <form id="form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                <div>
                    ...
                    Fecha:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="date" id="fsalida" name="fsalida" value="<?php echo date('Y-m-d') ?>"/>
                </div>
                <div>
